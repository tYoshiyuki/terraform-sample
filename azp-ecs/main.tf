...
resource "aws_ecs_task_definition" "ecs_task_definition_fargate" {
  container_definitions = templatefile("./container_definitions_fargate.json", {
    image_rul          = aws_ecr_repository.ecr_repository.repository_url
    awslogs_group_name = local.ecs_task_definition_fargate_name
  })
  family             = local.ecs_task_definition_fargate_name
  execution_role_arn = aws_iam_role.iam_role_execution_role.arn
  network_mode       = "awsvpc"
...
resource "aws_ecs_cluster" "ecs_cluster_ec2" {
  name = local.ecs_cluster_ec2_name
}

resource "aws_ecs_capacity_provider" "ecs_capacity_provider_ec2" {
...
resource "aws_ecs_cluster_capacity_providers" "ecs_cluster_capacity_providers_ec2" {
  cluster_name = aws_ecs_cluster.ecs_cluster_ec2.name

  capacity_providers = [aws_ecs_capacity_provider.ecs_capacity_provider_ec2.name]

  default_capacity_provider_strategy {
    base              = 0
    weight            = 1
    capacity_provider = aws_ecs_capacity_provider.ecs_capacity_provider_ec2.name
  }
}
...
resource "aws_iam_role" "iam_role_execution_role" {
  path                 = "/"
...
resource "aws_iam_role_policy" "task_execution" {
  role = aws_iam_role.iam_role_execution_role.id
...
resource "aws_iam_role_policy_attachment" "task_execution" {
  role       = aws_iam_role.iam_role_execution_role.name
